    ...
    for (int i=0; i<3; i++){
        for (int j=0; j<3; j++){
            if (matrix[i][j] != goal_matrix[i][j]){
                cout<<matrix[i][j]<<" "<<goal_matrix[i][j]<<" ";
                count++;
            }
        }
    }
...
int checkCost(vector<vector<int>> matrix, vector<vector<int>> goal_matrix){
    int count = 0;
...
void display(vector<vector<int>> matrix, vector<vector<int>> goal_matrix, int level, int type){
    ...
    if (type == 1){
        h = checkCost(matrix, goal_matrix);
    }
    else{
...
vector<vector<int>> transform_matrix(vector<vector<int>> matrix, vector<vector<int>> goal_matrix, set<vector<vector<int>>> &matrix_set, int level, int type){
    ...
    for (int i = 0; i <matrix_collection.size(); i++){
        ...
        if (pos == matrix_set.end()){
            ...
            if (type == 1){
                temp = checkCost(matrix_collection[i], goal_matrix);
            }
            else{
...
int main(){
    ...
    if (choice == 1){
        int heuristic = checkCost(matrix, goal_matrix);
        int level = 0;

        while (heuristic != 0){
            matrix = transform_matrix(matrix, goal_matrix, matrix_set, level+1, 1);
            heuristic = checkCost(matrix, goal_matrix);
            level++;
        }
    }
    else if (choice == 2){
